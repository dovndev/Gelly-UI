...
import { GellyButton } from "./GellyButton";

export interface GellyPricingTableProps {
  plans: { name: string; price: string; features: string[]; highlight?: boolean }[];
  className?: string;
}

export const GellyPricingTable: React.FC<GellyPricingTableProps> = ({
  plans,
  className,
}) => (
  <div className={clsx("flex gap-6 justify-center", className)}>
    {plans.map((plan, idx) => (
      ...
      >
        ...
        <GellyButton>Choose</GellyButton>
      </div>
    ))}
  </div>
);